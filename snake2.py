...
def your_score(score, stepen):
    value = score_font.render(f"Score: {score} (2^{stepen})", True, PURPLE)
    dis.blit(value, [0, 0])
...
def gameLoop():
    ...
    while not game_over:
        ...
        if x1 == foodx and y1 == foody:
            points_to_add = 2 ** stepen  # 2^0=1, 2^1=2, 2^2=4, 2^3=8, и т.д.
            score += points_to_add
            stepen += 1
            Length_of_snake += 1
            foodx = round(random.randrange(0, WIDTH - BLOCK_SIZE) / BLOCK_SIZE) * BLOCK_SIZE
